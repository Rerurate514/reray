...
export function SignedInMenu({
  ...
  const accountHref = sessionUser ? `/u/${sessionUser.username}` : '/my-schedule'

  return (
    <MenuShell trigger={<SignedInTrigger avatarUrl={avatarUrl} displayName={displayName} username={username} />}>
      <a class="flex items-center gap-2 border-b border-(--color-border) px-3 py-3 hover:bg-(--color-surface-muted)" href={accountHref}>
        {avatarUrl ? (
